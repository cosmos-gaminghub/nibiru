...
func (s *IntegrationTestSuite) SetupSuite() {
	...
	//
	// 1. Initialize Nibiru validator nodes.
	// 2. Create and initialize Nibiru validator genesis files (both chains)
...
func (s *IntegrationTestSuite) TearDownSuite() {
	if str := os.Getenv("NIBIRU_E2E_SKIP_CLEANUP"); len(str) > 0 {
...
func (s *IntegrationTestSuite) initGenesis(c *chain) {
	...
	for _, val := range c.validators {
		_ = writeFile(filepath.Join(val.configDir(), "config", "genesis.json"), bz)
	}
}
...
func (s *IntegrationTestSuite) runValidators(c *chain, portOffset int) {
	s.T().Logf("starting Nibiru %s validator containers...", c.id)
	...
	for i, val := range c.validators {
		runOpts := &dockertest.RunOptions{
			...
			Mounts: []string{
				fmt.Sprintf("%s/:/root/.nibiru", val.configDir()),
			},
			Repository: "cosmos/nibirud-e2e",
		}
		...
		s.T().Logf("started Nibiru %s validator container: %s", c.id, resource.Container.ID)
	}
	...
	s.Require().Eventually(
		...
		5*time.Minute,
		time.Second,
		"Nibiru node failed to produce blocks",
	)
}
...
func (s *IntegrationTestSuite) runIBCRelayer() {
	...
	tmpDir, err := ioutil.TempDir("", "nibiru-e2e-testnet-hermes-")
	s.Require().NoError(err)
	s.tmpDirs = append(s.tmpDirs, tmpDir)

	nibiruAVal := s.chainA.validators[0]
	nibiruBVal := s.chainB.validators[0]
	...
	s.hermesResource, err = s.dkrPool.RunWithOptions(
		&dockertest.RunOptions{
			...
			Env: []string{
				fmt.Sprintf("NIBIRU_A_E2E_CHAIN_ID=%s", s.chainA.id),
				fmt.Sprintf("NIBIRU_B_E2E_CHAIN_ID=%s", s.chainB.id),
				fmt.Sprintf("NIBIRU_A_E2E_VAL_MNEMONIC=%s", nibiruAVal.mnemonic),
				fmt.Sprintf("NIBIRU_B_E2E_VAL_MNEMONIC=%s", nibiruBVal.mnemonic),
				fmt.Sprintf("NIBIRU_A_E2E_VAL_HOST=%s", s.valResources[s.chainA.id][0].Container.Name[1:]),
				fmt.Sprintf("NIBIRU_B_E2E_VAL_HOST=%s", s.valResources[s.chainB.id][0].Container.Name[1:]),
			},
			Entrypoint: []string{
				"sh",
				"-c",
	...
	// create the client, connection and channel between the two Nibiru chains
	s.connectIBCChains()
}
