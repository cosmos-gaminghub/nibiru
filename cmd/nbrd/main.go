...
func main() {
	...
	rootCmd.AddCommand(
		...
		queryCommand(),
		txCommand(),
		keys.Commands(nibiru.DefaultNodeHome),
	)
...
func queryCommand() *cobra.Command {
	...
	nibiru.ModuleBasics.AddQueryCommands(cmd)
...
func txCommand() *cobra.Command {
	...
	nibiru.ModuleBasics.AddTxCommands(cmd)
	cmd.PersistentFlags().String(flags.FlagChainID, "", "The network chain ID")
...
func createSimappAndExport(
	...
	var nibiruApp *nibiru.NibiruApp
	if height != -1 {
		nibiruApp = nibiru.NewNibiruApp(logger, db, traceStore, false, map[int64]bool{}, "", cast.ToUint(appOpts.Get(server.FlagInvCheckPeriod)), encCfg, appOpts)

		if err := nibiruApp.LoadHeight(height); err != nil {
			return servertypes.ExportedApp{}, err
		}
	} else {
		nibiruApp = nibiru.NewNibiruApp(logger, db, traceStore, true, map[int64]bool{}, "", cast.ToUint(appOpts.Get(server.FlagInvCheckPeriod)), encCfg, appOpts)
	}
